				...
				<div class="mt-4">
					<div class="flex items-center text-sm">
						<span class="text-gray-500"
							>{{ ((stats.activeFlags / stats.totalFlags) * 100).toFixed(1) }}% of all flags</span
						>
					</div>
				</div>
			</UCard>
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<UCard>
					<template #header>
						<div class="flex justify-between items-center">
							...
							<UButton variant="ghost" size="sm" @click="navigateTo('/flags')"> View All </UButton>
						</div>
					</template>
			...
			<div>
				<UCard>
					...
					<div class="space-y-3">
						<UButton block icon="i-heroicons-plus" @click="navigateTo('/flags/new')">
							Create Feature Flag
						</UButton>

						<UButton block variant="outline" icon="i-heroicons-user-plus" @click="navigateTo('/users/new')">
							Add User
						</UButton>
...
const loadDashboardData = async () => {
	try {
		...
		if (groupsResponse.status === 'fulfilled' && groupsResponse.value.success) {
			...
			stats.value.averageGroupSize = groupsData.length > 0 ? Math.floor(Math.random() * 20) + 5 : 0 // Mock data
		}
	} catch (error) {
