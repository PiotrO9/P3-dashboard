...
export const useApi = () => {
  ...
  const flags = {
    ...
    update: (id: string, flag: Partial<FeatureFlag>) => apiCall<FeatureFlag>(`/flags/${id}`, { method: 'PUT', body: flag }),
    toggle: (id: string) => apiCall<FeatureFlag>(`/flags/${id}/toggle`, { method: 'PATCH' }),
    addRule: (id: string, rule: Partial<Rule>) => apiCall<Rule>(`/flags/${id}/rules`, { method: 'POST', body: rule }),
    deleteRule: (flagId: string, ruleId: string) => apiCall(`/flags/${flagId}/rules`, { method: 'DELETE', body: { ruleId } }),
    evaluate: (request: EvaluateRequest) => apiCall<{ result: boolean }>('/evaluate', { method: 'POST', body: request })
  }
  ...
  const groups = {
    ...
    create: (group: Partial<Group>) => apiCall<Group>('/groups', { method: 'POST', body: group, requiresAuth: true }),
    update: (id: string, group: Partial<Group>) => apiCall<Group>(`/groups/${id}`, { method: 'PUT', body: group, requiresAuth: true }),
    ...
    addUser: (groupId: string, userId: string) => apiCall(`/groups/${groupId}/users`, { method: 'POST', body: { userId }, requiresAuth: true }),
    removeUser: (groupId: string, userId: string) => apiCall(`/groups/${groupId}/users/${userId}`, { method: 'DELETE', requiresAuth: true })
  }

  // Auth API
  const auth = {
    login: (credentials: LoginCredentials) => apiCall<{ user: User; token: string }>('/users/login', { method: 'POST', body: credentials }),
